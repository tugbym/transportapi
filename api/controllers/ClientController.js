...
module.exports = {
    ...
    //Log the client in with the oauth2 client passport strategy.
    login: function(req, res) {
        var base = 'http://' + req.headers.host;
        passport.authenticate(['oauth2-client-password'], function(err, client) {
            
            //No errors, but no client found.
            if(!err && !client) {
                res.setHeader("Content-Type", "application/vnd.collection+json");
                return res.status(404).json(cj.createCjError(base, "Client not found", 404));
                
            //Error authenticating the client.
            } else if(err) {
                res.setHeader("Content-Type", "application/vnd.collection+json");
                return res.status(500).json(cj.createCjError(base, err, 500));
                
            //No problems, so set the session state.
            } else {
                req.logIn(client, function(err) {
                    
                    //Error setting session state.
                    if(err) {
                        res.setHeader("Content-Type", "application/vnd.collection+json");
                        return res.status(500).json(cj.createCjError(base, err, 500));
    ...
    //Get the logged in clients information
    readCurrent: function(req, res) {
        var base = 'http://' + req.headers.host;
        var id = req.user.id;
        ...
        Client.findOne({
            ...
            //No problems.
                ...
                res.setHeader("Content-Type", "application/vnd.collection+json");
                res.status(200).json(cj.createCjTemplate(base, client));
            }
        });
    },
    ...
    updateCurrent: function(req, res) {
        ...
        var acceptedEditInputs = ['redirectURI'];
        var newDoc = {};
        for(request in req.body) {
            ...
            if(acceptedEditInputs.indexOf(request) == -1) {
                res.setHeader("Content-Type", "application/vnd.collection+json");
                return res.status(400).json(cj.createCjError(base, "You may only edit your redirect URI.", 400));
            }
        ...
        //Update the model.
        Client.update({id: id}, newDoc).exec(function(err, updatedDoc) {
            
            //No problems.
            if(!err && updatedDoc[0]) {
                res.setHeader("Location", base + "/api/client/" + id);
    ...
    //Delete the logged in client
    deleteCurrent: function(req, res) {
        var base = 'http://' + req.headers.host;
        
        //Ensure clients session state is deleted first.
        req.logout();
        var id = req.user.id;
