...
module.exports = {
  ...
  update: function (req, res) {
      ...
      .exec(function(err, updatedDoc) {
          if (!err) {
              res.status(200).json({message: "Bus Stop updated: " + updatedDoc[0]});
          } else {
              res.status(500).json({message: "Could not update bus stop: " + err});
          }
      });
  },
  delete: function (req, res) {
    ...
    BusStop.findOne({id: id}, function(err, doc) {
        if (!err && doc) {
            BusStop.destroy(doc)
            .exec(function(err) {
                if (!err) {
                    res.status(200).json({message: "Bus Stop successfully removed."});
                } else {
                    res.status(403).json({message: "Could not delete bus stop: " + err});
                }
            })
        } else if (!err) {
            res.status(404).json({message: "Could not find bus stop."});
        } else {
            res.status(403).json({message: "Could not delete bus stop: " + err});
        }
    });
